...
#     num_req_sigs = stack.pop()
#     signatures = []
#     for i in range(num_req_sigs):
...
class OpTest(TestCase):
    ...
    def test_op_checkmultisig(self):
        ...
        self.assertEqual(decode_num(stack[0]), 1)
        stack = [b'', sig1, sig2, b'\x02', sec2, sec1, b'\x02']
        self.assertTrue(op_checkmultisig(stack, z))
        self.assertEqual(decode_num(stack[0]), 1)
        stack = [b'', sig2, sig1, b'\x02', sec1, sec2, b'\x02']
        self.assertTrue(op_checkmultisig(stack, z))
        self.assertEqual(decode_num(stack[0]), 1)
        stack = [b'', sig2, sig1, b'\x02', sec2, sec1, b'\x02']
        self.assertTrue(op_checkmultisig(stack, z))
        self.assertEqual(decode_num(stack[0]), 1)
        
        #NEGATIVE TESTING
        stack = [b'', sig1, sig2, b'\x02', sec1, b'\x01']
        #missing sec2
        self.assertFalse(op_checkmultisig(stack, z))
        stack = [b'', sig1, sig2, b'\x02', sec2, b'\x01']
        #missing sec1
        self.assertFalse(op_checkmultisig(stack, z))
        #make the sec2 wrong by changing last '1' to '0'
        sec2 = bytes.fromhex('03b287eaf122eea69030a0e9feed096bed8045c8b98bec453e1ffac7fbdbd4bb70')
        stack = [b'', sig1, sig2, b'\x02', sec1, b'\x01']
        self.assertFalse(op_checkmultisig(stack, z))
